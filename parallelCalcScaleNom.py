...
def calcScaleNom(inFile):

    inList = ROOT.vector('string')()
    ...
    print 'here'
    df      = ROOT.RDataFrame("nominal",inList)     #nominal tree dataframe
    allCols = df.GetColumnNames()
    sdf     = ROOT.RDataFrame("sumWeights",inList)  #sumweights tree dataframe
...
pool = ROOT.ROOT.TProcessExecutor()
pool.Map(calcScaleNom, linelist)
#Parallel(n_jobs=2)(delayed(calcScaleNom)(inFile) for inFile in linelist)
